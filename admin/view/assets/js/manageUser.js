...
function load_table() {
    $.ajax({
        url: "../controller/controllerManageUser.php",
        type: "post",
...
function toggleStatus(userId) {
    ...
    $.ajax({
        url: '../controller/controllerManageUser.php',  // Path to your PHP script
        method: 'POST',
        data: {
            id: userId,
            action: "active",
...
$(document).on('click','#btnEdit', function() {
    ...
    $.ajax({
        ...
        success: function(response){
            var data = JSON.parse(response);
            if (data['status'] == "success") {
                    $('#viewUserProfile #first_name').val(data.record.first_name);
                    $('#viewUserProfile #last_name').val(data.record.last_name);
                    $('#viewUserProfile #userId').val(data.record.id); // Assuming you have a dropdown or input field for status
                    $('#viewUserProfile #btnSave').text("Update");
                    $('#viewUserProfile #btnSave').attr('name', "btnUpdate");
                    $('#viewUserProfile #btnSave').attr('id', "btnUpdate");
